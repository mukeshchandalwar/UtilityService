...
# Copy the JAR file
ARG JAR_FILE=./target/utilityservice-0.0.1-SNAPSHOT.jar
COPY ${JAR_FILE} /usr/app/utilityservice.jar
...
COPY --from=builder /usr/app/utilityservice.jar /usr/app/utilityservice.jar

# Set the entry point
ENTRYPOINT ["/usr/lib/java-17-openjdk/bin/java", "-jar", "/usr/app/utilityservice.jar"]